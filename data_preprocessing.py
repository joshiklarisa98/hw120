...
# Training Dataset: 
# create a list of stem words, classes and pattern_word_tags_list
stem_words, classes, pattern_word_tags_list = create_bot_corpus(words, classes)
# Input Text----> as Bag of Words 
def create_input_data(pattern_word_tags_list, stem_words):
    # create an empty list of input data
    input_data = []
    # create an empty list of output data
    output_data = []
    # create a list of unique tags
    tags = []
    # loop through the pattern_word_tags_list
    for pattern_words, tag in pattern_word_tags_list:  
        # create an empty list of bag of words  
        bag = []
        # create a list of stemmed words  
        pattern_words = get_stem_words(pattern_words, ignore_words)
        # create a bag of words by looping through the stemmed words
        for word in stem_words:
            if word in pattern_words:
                ...
                bag.append(0)
# Tags-----------> as Label
# Create labels for each tag
labels = []
for intent in intents['intents']:
    tag = intent['tag']
    labels.append(tag)# Tags-----------> as Label
